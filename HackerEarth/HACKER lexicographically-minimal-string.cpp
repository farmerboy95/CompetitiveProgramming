/*
    ...
    Idea:
        - Use DSU to solve.
        - Merge all equivalent pairs, then for each component, find the smallest character.
        Attach that character to each character in the component for quick referral.
        - For each character in string C, replace it by the smallest character stored above.
*/
...
int main() {
    ...
    FOR(i,0,SZ(groups)-1) {
        int Min = groups[i][0];
        FOR(j,1,SZ(groups[i])-1) {
            ckmin(Min, groups[i][j]);
        }
        FOR(j,0,SZ(groups[i])-1) {
            smallest[groups[i][j]] = Min;
        }
    }

    FOR(i,0,SZ(st)-1) {
        int x = st[i] - 'a';
        st[i] = char(smallest[x] + 'a');
    }
    cout << st;
    return 0;
}
