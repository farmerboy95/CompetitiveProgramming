...
int main() {
    ...
    DisjointSetUnion dsu(n), origin(n);

    int xorSum = 0;
    remainingEdges = (ll) n * (n-1) / 2 - m;
    ...
    // connect the unassigned edges (reversed graph)
    FOR(i,1,n)
        if (avail.count(i)) dfs(i, dsu);

    // assign xorSum weight to one of the unused edges
    if (remainingEdges) xorSum = 0;
    ...
    for (auto [c, u, v] : edges) {
        if (!dsu.same(u-1, v-1)) {
            ...
            res += c;
            dsu.merge(u-1, v-1);
            origin.merge(u-1, v-1);
        } else if (!origin.same(u-1, v-1)) {
            // type 3
            // replace an unassigned edge in that cycle with this type 3 edge
            xorSum = min(xorSum, c);
            origin.merge(u-1, v-1);
        }
    }
