/*
    ...
    Status: AC
    Idea:
        - Similar as https://codeforces.com/blog/entry/72243
...
// find depth of a subtree rooted at 1
...
// this dfs is for getting fromPar[v], which is maxD[par[v]] when the tree is rooted at v
...
int main() {
    ...
    // calculate odd case and one case of even case
    FOR(i,1,n) {
        w.clear();
        FOR(j,0,SZ(a[i])-1) {
            int v = a[i][j];
    ...
    FOR(u,1,n) {
        tmpMap.clear();
        int maxU = 0, sumU = 0;
        // add information of node u to map
        FORALL(it, ma[u]) {
            tmpMap[it->FI] += it->SE;
            sumU += it->SE;
            maxU = max(maxU, it->FI);
        ...
        FOR(j,0,SZ(a[u])-1) {
            ...
            if (v == par[u]) continue;
            int maxV = 0, sum = sumU;
            // add information of node v to map
            FORALL(it, ma[v]) {
                tmpMap[it->FI] += it->SE;
                sum += it->SE;
                maxV = max(maxV, it->FI);
            ...
            // calculate the result just like in editorial (way number 2)
            FORALL(it, tmpMap) {
