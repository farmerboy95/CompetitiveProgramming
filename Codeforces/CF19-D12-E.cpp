...
int n, m, col[MAXN], isBipartite[MAXN], groupNum[MAXN], cntGroup, root[MAXN], dp[MAXN], lvl[MAXN], par[MAXN], dpC[MAXN];
pi edges[MAXN];
vi a[MAXN];
map<pi, int> ma;

void coloring(int u, int gr) {
    groupNum[u] = gr;
    TRAV(v, a[u]) {
        if (!col[v]) {
            col[v] = -col[u];
...
int main() {
    ...
    FOR(i,1,cntGroup) isBipartite[i] = 1;

    vi contradictEdges;
    FOR(i,1,m) {
        int u = edges[i].FI, v = edges[i].SE;
        if (col[u] == col[v]) {
            isBipartite[groupNum[u]] = 0;
            contradictEdges.push_back(i);
        }
    }

    int numNonBipartite = 0;
    FOR(i,1,cntGroup) numNonBipartite += (isBipartite[i] == 0);

    if (numNonBipartite == 0) {
        // full bipartite
        cout << m << "\n";
        FOR(i,1,m) cout << i << ' ';
        return 0;
    ...
    if (numNonBipartite > 1) {
        // more than one non-bipartite
        cout << 0 << "\n";
        return 0;
    }
    ...
    int treeRoot = root[group];
    vi res;
    if (SZ(contradictEdges) == 1) {
        res.push_back(contradictEdges[0]);
    }
