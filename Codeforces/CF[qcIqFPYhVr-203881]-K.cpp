/*
    ...
    Idea:
        ...
        mask, we do SOS DP again in reverse to exclude the ones that are not equal
        to mask. Then we just calculate result on each mask, adding dp[mask] * mask^3.
        - Complexity O(n + k * 2^k) (k = 17, 2^k > max a[i])
*/
...
int n, a[MAXN], cnt[1<<MAXK];
mi dp[1<<MAXK], mu[MAXN+1];
...
int main() {
    ...
    FOR(mask,0,(1<<MAXK)-1) {
        dp[mask] = mu[cnt[mask]] - 1;
    }

    FOR(i,0,MAXK-1) {
        FOR(mask,0,(1<<MAXK)-1) {
            if (mask & (1<<i)) {
                mi p = dp[mask] + dp[mask ^ (1<<i)] + dp[mask] * dp[mask ^ (1<<i)];
                dp[mask] = p;
            }
        }
    }
