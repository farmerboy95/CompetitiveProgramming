/*
    ...
    Status: AC
    Idea:
        - Another problem code of CF405-D2-E
        - Consider a connected component, let's say it has x nodes and y edges, we claim
        that we can create floor(y/2) pairs from this component. Let's prove this.
        - First, we build a DFS tree. Consider an arbitrary leaf node U of that tree. Here
        we have a number K of back edges. If K is even, we can create K/2 pairs from it.
        Otherwise, we can create (K-1)/2 pairs, the last back edge can be used to create
        another pair with U and parent of U in the DFS tree. So after we consider U, we
        ...
        create floor(y/2) pairs.
        - Print sum pairs from all components.
        - Time: O(n)
*/
