/*
    ...
    Idea:
        ...
        - Let dp[i][x] = smallest number of groups that can be achieved after coloring a[1] .. a[i]
...
int main() {
    ...
    FOR(i,2,n) {
        ...
        // x != a[i]
        int rem = min(st.get(1,0,st.N-1,0,now-1), st.get(1,0,st.N-1,now+1,st.N-1)) + 1;

        // update dp[i][a[i]-1]
        int g = st.get(1,0,st.N-1,pre,pre);
        st.update(1,0,st.N-1,pre,pre,min(w, rem) - g);
