...
int n, m, lvl[MAXN], cycleId[MAXN], goingDown[MAXN], parent[MAXN][20], d[MAXN], cnt[MAXN];
vpi a[MAXN];
vi A[MAXN];
vpi edges;
mi mu[MAXN];
...
int main() {
    ...
    vi nodes;
    TRAV(x, edges) {
        int u = cycleId[x.FI], v = cycleId[x.SE];
        nodes.push_back(u);
        nodes.push_back(v);
        if (u == v) continue;
        A[u].push_back(v);
    ...
    d[root] = 1;
    dfs2(root, 0);
    TRAV(i, nodes) dbg(i, cnt[i]);

    FOR(j,1,18)
        TRAV(i, nodes) parent[i][j] = parent[parent[i][j-1]][j-1];

    mu[0] = 1;
    FOR(i,1,n) mu[i] = mu[i-1] * 2;
