/*
    ...
    Status: AC
    Idea: https://codeforces.com/blog/entry/16468
*/
...
// as it's quite complicated to get max, i decided to sort them all, luckily got AC
...
void RMQ() {
    // RMQ getting 2 largest elements of a range
    FOR(i,1,N) {
        ML[i][0] = II(i, -1);
        MR[i][0] = II(i, -1);
    }
...
ll consider(int l, int r) {
    ...
    II x = getL(l, r);
    II y = getR(l, r);
    // beware of the case when LMax[x] == RMax[y] and x == y
    if (x.FI == y.FI) {
        ll res = 0;
...
int main() {
    ...
    while (m--) {
        int u, v;
        cin >> u >> v;
        // get endpoints of the segments without children
        u--;
        if (u < 1) u = n;
        v++;
        if (v > n) v = 1;
        ...
        // consider all segments without children
        int pre = 1;
        ll res = 0;
        FOR(i,0,SZ(a)-1) {
