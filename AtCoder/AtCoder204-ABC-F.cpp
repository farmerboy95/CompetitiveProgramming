/*
    ...
    Status: AC
    Idea:
        - https://atcoder.jp/contests/abc204/editorial/2026
*/
...
void f(int mask, int u) {
    ...
    if (mask & (1<<u)) {
        ...
        f(mask, u+1);
        f(mask ^ (1<<u), u+1);
        if (u + 1 < n && !(mask & (1<<(u+1)))) f(mask, u+2);
    }
}
...
int main() {
    ...
    FOR(mask,0,(1<<n)-1) {
        ...
        f(mask, 0);
        FOR(i,0,(1<<n)-1) A.a[i][mask] = cnt[i];
    }

    Matrix B(1<<n);
    FOR(i,0,(1<<n)-1) B.a[i][i] = 1;
