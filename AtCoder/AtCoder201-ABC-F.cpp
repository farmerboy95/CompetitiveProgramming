/*
    ...
    Status: AC
    Idea:
        - https://atcoder.jp/contests/abc201/editorial/1838
*/
...
int n, p[MAXN], a[MAXN], b[MAXN], c[MAXN], pos[MAXN];
...
int main() {
    ...
    FOR(i,1,n) L[i] = L[i-1] + min(a[i], b[i]);
    FORE(i,n,1) R[i] = R[i+1] + min(a[i], c[i]);

    FOR(i,1,n) pos[p[i]] = i;
 
    SegmentTreeLazy seg(n+1);
    seg.update(1,0,seg.N-1,0,0,-1e17);

    FOR(i,1,n) {
        dp[i] = min(L[i-1], seg.get(1,0,seg.N-1,0,pos[i]-1));

        seg.update(1,0,seg.N-1,0,seg.N-1,a[i]);

        ll x = seg.get(1,0,seg.N-1,pos[i],pos[i]);
        seg.update(1,0,seg.N-1,pos[i],pos[i],-x+dp[i]);
    }

    ll res = 1e18;
    FOR(i,0,n) res = min(res, dp[i] + R[i+1]);
    cout << res; 
    return 0;
}
