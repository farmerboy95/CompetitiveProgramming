/*
    ...
    Status: AC
    Idea:
        - Connect n rows with source, m columns with the sink
        - For each piece, we connect the rows and the columns with that piece
        - But it's not enough, we have to make sure that we choose 1 cell by creating
...
int main() {
    ...
    int source = 0;
    pi rowRow = {1, n};
    pi kRow = {rowRow.SE+1, rowRow.SE+k};
    pi k2Row = {kRow.SE+1, kRow.SE+k};
    pi colRow = {k2Row.SE+1, k2Row.SE+m};
    int sink = colRow.SE + 1;

    Dinic dinic(sink+1);

    FOR(i,rowRow.FI,rowRow.SE) dinic.addEdge(source, i, 1);
    FOR(i,colRow.FI,colRow.SE) dinic.addEdge(i, sink, 1);

    FOR(o,0,k-1) {
        dinic.addEdge(kRow.FI+o, k2Row.FI+o, 1);
    }
    ...
    FOR(o,0,k-1) {
        ...
        l--; r--; u--; v--;
        FOR(i,l,r) {
            dinic.addEdge(rowRow.FI + i, kRow.FI+o, 1);
        }
        FOR(i,u,v) {
            dinic.addEdge(k2Row.FI+o, colRow.FI + i, 1);
        }
    }
