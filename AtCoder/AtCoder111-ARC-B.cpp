/*
    ...
    Status: AC
    Idea:
        - Create a graph with edges are the input data
        - We can choose a card in 2 by choosing the direction of an edge
        - Easily see that if a connected component (with x nodes) is a tree => we can get at most x-1 
        distinct numbers at the end.
        - Otherwise, we can get at most x numbers at the end.
        - Use DSU to solve.
*/
...
struct DisjointSetUnion {
  public:
    ...
    DisjointSetUnion(int n) : _n(n), parent_or_size(n, -1), num_of_edges(n, 0) {}
    ...
    int merge(int u, int v) {
        ...
        num_of_edges[x] += num_of_edges[y] + 1;
        parent_or_size[y] = x;
        return x;
    }
    ...
    int get() {
        ...
        FOR(i,0,_n-1) {
            if (parent_or_size[i] < 0) {
                int x = -parent_or_size[i];
                int y = num_of_edges[i];
                if (y >= x) res += x;
                else res += x-1;
            }
        }
  ...
  private:
    int _n;
    vector<int> parent_or_size, num_of_edges;
};
