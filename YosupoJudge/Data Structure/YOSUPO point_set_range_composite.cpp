/*
    ...
    Status: AC
    Idea:
        - Use segment tree, each node stores a linear function
*/
...
struct Linear {
    mi a, b;
    Linear(mi _a = 0, mi _b = 0) : a(_a), b(_b) {}
    mi eval(mi x) {
        return a * x + b;
    }
};

inline Linear op(Linear a, Linear b) {
    return Linear(a.a * b.a, a.b * b.a + b.b);
}
inline Linear e() { return Linear(1, 0); }
inline Linear mapping(Linear a, Linear b) {
    return b;
}
...
int main() {
    ...
    SegmentTree<Linear, op, mapping, e, e> seg(a);

    while (q--) {
        int ch, l, r, c;
        cin >> ch >> l >> r >> c;
        if (ch == 0) {
            Linear cur = Linear(r, c);
            seg.update(1, 0, n-1, l, cur);
        } else {
            r--;
            Linear res = seg.get(1, 0, n-1, l, r);
            cout << int(res.eval(c)) << "\n";
        }
    }
    return 0;
