...
int main() {
    ...
    while (cin >> n >> m) {
        ...
        // limit the flow at d only
        dinic.addEdge(0, 1, d, 0);

        FOR(i,0,m-1) {
            int u = get<0>(a[i]), v = get<1>(a[i]), c = get<2>(a[i]);
            dinic.addEdge(u, v, k, c, 1);
        }

        dinic.getFlow(0, n);

        if (dinic.flow != d) cout << "Impossible.\n";
        else cout << dinic.cost << "\n";
    }
    return 0;
}
