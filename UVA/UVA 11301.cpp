/*
    ...
    Idea:
        - Min Cost Max Flow
        - Each character has 2 nodes: in-node and out-node
        - There is an edge from in-node to out-node, capacity = 1 and cost = toll of the character.
        - At each character, you can go to at most 4 adjacent characters, so you can move from the out-node
        of the current one to the in-node of the new one with no cost
        - Connect the source with in-nodes of @ characters
        - Connect the out-nodes of the ends of the lines to the sink
*/
...
int main() {
    ...
    while (cin >> n && n) {
        FOR(i,0,4) cin >> s[i];
        DinicMCF dinic(5 * n * 2 + 2);
        int source = dinic.N - 2, sink = dinic.N - 1;

        FOR(i,0,4) {
            FOR(j,0,n-1) {
                int toll = s[i][j] == '@' ? 0 : s[i][j] - '0';
                dinic.addEdge(inNode(i,j), outNode(i,j), 1, toll);

                FOR(k,0,3) {
                    int u = i + dx[k], v = j + dy[k];
                    if (!isInside(u, v)) continue;
                    dinic.addEdge(outNode(i,j), inNode(u,v), 1, 0);
                }
            }
            if (s[i][0] == '@') {
                dinic.addEdge(source, inNode(i, 0), 1, 0);
            }
            dinic.addEdge(outNode(i, n-1), sink, 1, 0);
        }
