...
int main() {
    ...
    int n, m;
    while (cin >> n >> m && n + m) {
        HungarianAlgo hungarian(m);

        FOR(i,1,n) FOR(j,1,m) {
            ld x;
            cin >> x;
            hungarian.setCost(i-1, j-1, x);
        }
        FOR(i,n+1,m) FOR(j,1,m) hungarian.setCost(i-1, j-1, 0);

        hungarian.getMatching();

        cout << fixed << setprecision(2) << hungarian.getMatchingValue() / n + EPS << "\n";
    }
    return 0;
}
