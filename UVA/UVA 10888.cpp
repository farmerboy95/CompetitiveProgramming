/*
    ...
    Idea:
        ...
        - It does not matter that one B should go after another B
*/
...
int main() {
    ...
    while (te--) {
        ...
        FOR(i,0,n-1)
            FOR(j,0,m-1)
                if (ch[i][j] == 'B') b.push_back({i, j});
                else if (ch[i][j] == 'X') x.push_back({i, j});
        
        int N = SZ(b);
        HungarianAlgo hungarian(N);
        FOR(i,0,N-1) {
            bfs(b[i].FI, b[i].SE);
            FOR(j,0,N-1) {
                int val = d[x[j].FI][x[j].SE];
                hungarian.setCost(i, j, val);
            }
        }
