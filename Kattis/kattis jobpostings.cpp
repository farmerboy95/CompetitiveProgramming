/*
    ...
    Idea:
        - Min Cost Max Flow
        - The problem is asking about max cost, we just use negative costs to make it min cost.
*/
...
int n, m, a[MAXN], y[MAXN], c[MAXN][4];
...
int main() {
    ...
    while (cin >> n >> m && n + m) {
        ...
        FOR(i,1,m) dinic.addEdge(source, i, 1, 0);
        FOR(i,1,n) dinic.addEdge(m+i, sink, a[i], 0);

        FOR(i,1,m) {
            FOR(j,0,3) {
                int cost = y[i] * 4 - j;
                int idx = c[i][j] + 1;
                dinic.addEdge(i, m+idx, 1, -cost);
            }
        }
