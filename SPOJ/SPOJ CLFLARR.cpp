/*
    ...
    Status: AC
    Idea:
        - We use DSU to solve, leader of one element in a component is the nearest element
        to the right that is not painted yet.
        - Initially, each element is in one component containing itself only.
        - We proceed queries from the last one to the first one. In each query (L, R, C), we
        start from element L, we find the leader of the component containing L, which is the
        nearest element x to the right of L that is not painted yet. Paint that one with color
        C if x <= R.
        - Now, how to go to the next component? It's element x+1, we then merge component
        containing x and component containing x+1, then continue to go from x+1.
        - Add path compression to DSU to avoid time wasting.
        - Remember to create DSU with n+1 elements, since we need a redundant element n+1 to
        be the leader if prefix of the array is painted.
*/
