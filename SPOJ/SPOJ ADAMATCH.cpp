/*
    ...
    Status: AC
    Idea:
        - Same idea as MAXMATCH problem (SPOJ/MAXMATCH.cpp)
*/
...
int main() {
    ...
    for (char ch : dict) {
        ...
        b.clear();
        FOR(i,0,SZ(t)-1) b.push_back((t[i] == ch));

        A.clear();
        B.clear();
        FOR(i,0,SZ(a)-1) A.push_back(a[i]);
        FOR(i,0,SZ(b)-1) B.push_back(b[i]);
        reverse(ALL(A));
    ...
    int Max = -1;
    FOR(i,0,SZ(s)-SZ(t)) Max = max(Max, res[i]);
    cout << SZ(t)-Max;
    return 0;
}
