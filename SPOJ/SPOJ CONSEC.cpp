/*
    ...
    Status: AC
    Idea:
        - Do it reversely, use DSU to merge components
*/
...
int main() {
    ...
    FOR(o,1,te) {
        ...
        FOR(i,1,q) {
            ...
            cin >> ch >> u;
            if (ch == 1) {
                queries.push_back({ch, u});
            } else {
                queries.push_back({ch, u, s[u] - 'A'});
                s[u] = '#';
            }
        }
        ...
        vi res;
        FORE(i,q-1,0) {
            int ch = queries[i][0], u = queries[i][1];
            if (ch == 1) {
                res.push_back(dsu.size(u));
            } else {
                char w = queries[i][2] + 'A';
                s[u] = w;
                if (u-1 >= 0 && s[u-1] == w) {
                    dsu.merge(u, u-1);
                }
