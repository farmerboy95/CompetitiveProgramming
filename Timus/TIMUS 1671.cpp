/*
    ...
    Status: AC
    Idea:
        - The problem asked us to find the number of connected components after breaking 
        a specific edge.
        - We maintain a DSU here. Before processing, we add all the edges that are not in
        query list to DSU. Then we process the queries in the reversed order. At each query,
        we add the edge to DSU, remember to save the current number of connected components
        because it's the result after that query.
        - If we can merge 2 components, reduce the number.
*/
...
int main() {
    ...
    int curSize = dsu.groups().size();
    FORE(i,SZ(queries)-1,0) {
        res.push_back(curSize);
        int u = edges[queries[i]].first, v = edges[queries[i]].second;
